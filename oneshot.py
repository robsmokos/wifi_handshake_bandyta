...
def ifaceUp(iface, down=False):
    """Bring network interface up or down. Works on Linux (using ip) and Windows (using netsh)."""
    if os.name == 'nt':
        # Windows: use netsh to enable/disable the interface
        state = 'DISABLED' if down else 'ENABLED'
        cmd = ['netsh', 'interface', 'set', 'interface', iface, f'admin={state}']
    else:
        # Linux/Unix: use ip link command
